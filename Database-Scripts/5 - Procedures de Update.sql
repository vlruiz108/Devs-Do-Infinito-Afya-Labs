...
CREATE PROCEDURE sp_updateClient(p_cep int, p_logradouro VARCHAR(45), p_numero VARCHAR(10), 
	p_bairro VARCHAR(45), p_localidade VARCHAR(45), p_uf VARCHAR(2), p_id_endereco INT, 
...
BEGIN
    ...
    bairro = p_bairro, localidade = p_localidade, uf = p_uf WHERE id_endereco = p_id_endereco;
	UPDATE tbl_clientes SET cpf = p_cpf, nome = p_nome, telefone = p_telefone, celular = p_celular,
    email = p_email, tipo_sanguineo = p_tipo_sanguineo, FK_id_endereco = p_id_endereco 
...
BEGIN
    ...
    celular = p_celular, email = p_email, FK_id_endereco = p_id_endereco, FK_id_profissao = p_FK_id_profissao 
...
BEGIN
	UPDATE tbl_atendimentos SET data_agendamento = p_data_agendamento, data_atendimento = p_data_atendimento,
...
DELIMITER $$
CREATE PROCEDURE sp_updateMedReg(p_data_abertura DATE, p_FK_id_cliente INT, p_id_prontuario INT)
BEGIN
	UPDATE tbl_prontuarios SET data_abertura = p_data_abertura, FK_id_cliente = p_FK_id_cliente 
...
BEGIN
	UPDATE tbl_prontuario_historicos SET descricao = p_descricao, FK_atendimento_historico = p_FK_atendimento_historico
